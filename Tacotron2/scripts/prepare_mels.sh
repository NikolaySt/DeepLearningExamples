...
if [ $(ls $DATADIR/mels | wc -l) -ne 13100 ]; then
    python preprocess_audio2mel.py --dataset-path "$DATADIR" --wav-files "$TRAINLIST" --mel-files "$TRAINLIST_MEL"
    python preprocess_audio2mel.py --dataset-path "$DATADIR" --wav-files "$TESTLIST" --mel-files "$TESTLIST_MEL"
    python preprocess_audio2mel.py --dataset-path "$DATADIR" --wav-files "$VALLIST" --mel-files "$VALLIST_MEL"	
fi	
